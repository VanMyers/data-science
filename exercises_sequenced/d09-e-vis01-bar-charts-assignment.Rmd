...
`geom_col()`. Use that knowledge to convert the following `geom_bar()` plot into the same visual using `geom_col()`.
...
**Observations**: - The `hwy` values are unrealistically high. This is because multiple observations are stacked on top of each other.
...
**Observations**: - There are the fewest 2 seaters and the most SUV's. Both sedan categories are also very popular
...
**Observations**: - All 2 seaters are rear wheel drive. Aside from some SUVs and subcompacts no other vehicles are! All pickups are 4wd and all the remaining SUVs (that aren't rwd) are. Subcompact is the only category with all three drive types. Minivans are all front wheel drive. Sedans are mostly fwd with a few awd offerings, mostly in the compact segment.

**Compare `v1` and `v2`**:

-   Which visualization---`v1` or `v2`---enabled you to make more observations? `v2`
-   What was the difference between `v1` and `v2`? `v2` used `fill` to visualize an additional variable, `drv`

### **q4** The following code has a bug; it does not do what its author intended.

Identify and fix the bug. What does the resulting graph tell you about the relation between `manufacturer` and `class` of cars in this dataset?
...
**Observations** - Jeep, Land Rover, Lincoln, and Mercury only have SUV's in this dataset. Honda only has subcompacts. All the 2seaters are chevy mdoels. Pontiac only made midsize cars and audi made mostly compacts with a few midsize. Hyundai split equally between subcompact and midsize.

### **q5** The following graph is hard to read. What other form of faceting would

make the visual more convenient to read? Modify the code below with your suggested improvement.

```{r q5-task}
mpg %>%
  ggplot(aes(x = cyl)) +
  geom_bar() +
  facet_wrap(~ manufacturer) # this stil is bad

# mpg %>%
#   ggplot(aes(x = cyl, fill = manufacturer)) +
#   geom_bar()
```

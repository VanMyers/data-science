...
*Reading*: [Separate and Unite Columns](https://rstudio.cloud/learn/primers/4.2) *Topics*: Welcome, separate(), unite(), Case study *Reading Time*: \~30 minutes

*Notes*: - I had trouble running the Case study in my browser. Note that the `who` dataset is loaded by the `tidyverse`. You can run the Case study locally if you need to! - The case study uses `gather` instead of `pivot_longer`; feel free to use `pivot_longer` in place.
...
The Case study was already a fair bit of work! Let's do some simple review with `separate` and `unite`.
...
## TODO: pivot and unite here!
  ...
  pivot_wider(names_from = parent2, values_from = offspring)

...
-   What is the mean departure delay time, among available values? 12.64 minutess
...
# Additional peak to investigate negative values
flights %>%
  arrange(dep_delay)
```
...
-   Which carriers are at the top of both `mean` and `median` departure delay? F9, FL, and WN
-   Which carriers differ in the two lists? EV and YV only appear in the mean list and UA and VX only appear in the median list
-   What do negative values of `dep_delay` mean? I assume that the flight left early. This is supported by the fact few flights have `dep_delay` less than -30. You would expect flights to leave a coupple minutes early because of efficient boarding but not to surprise passengers by leaving hours early.
...
quartiles <-
  v_test_iqr %>%
  quantile(probs = (0:4)/4, na.rm = TRUE)
...
-   Which carriers are at the top of both `sd` and `IQR` departure delay? F9, FL, YV, and EV
-   Which carriers differ in the two lists? HA has the highest sd but does not appear in the list of top IQR. 9E appears only in the list of top IQR
-   Which carrier has the biggest difference between its `sd` and `IQR`? What might this mean? HA has a far greater difference between sd and IQR than any other carrier. This likely means it has the most skewed distribution of delays or a couple massive outliers. Two exploratory plots reveal that the distribution is skewed and there is a single outlier with a delay over 1000 minutes.
...
```{r ex-cor-line}
slope <- -1 # Play with this value; observe the correlation
df_line <-
...
-   For what values of `slope` is the correlation positive? `slope > 0`
-   For what values of `slope` is the correlation negative? `slope < 0`
...
-   What do you predict the correlations to be? Basically 0
-   What are the actual correlations? As predicted

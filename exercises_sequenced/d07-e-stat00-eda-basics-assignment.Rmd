...
*Reading*: [Exploratory Data Analysis](https://rstudio.cloud/learn/primers/3.1) *Topics*: (All topics) *Reading Time*: \~45 minutes
...
**Here's a hypothesis**: `Ideal` is the "best" value of `cut` for a diamond. Since an `Ideal` cut seems more labor-intensive, I hypothesize that `Ideal` cut diamonds are less numerous than other cuts.
...
**Observations**: - Ideal is the most common cut. Each lower grade becomes progressively rarer.
...
**Observations**: - This plot shows no clear correlation between cut and price
...
**Observations**: - Ideal diamonds have the lowest average price

## Unraveling Hypothesis 2

Upon making the graph in **q2.2**, I was very surprised. So I did some reading on diamond cuts. It turns out that some gemcutters [sacrifice cut for carat](https://en.wikipedia.org/wiki/Diamond_(gemstone)#Cut). Could this effect explain the surprising pattern above?
...
**Observations**: - The largest diamonds are low quality cuts, but otherwise there is not substantial correlation between carat and cut. This supports this hypothesis on the extreme. The average fair diamond is also larger which may support this hypothesis, however, this could also reflect purchasing habits not just cutting decisions. i.e. small and low quality cut is very undesirable so only larger diamonds are bought with fair cuts.

## Footnotes

[1] Don't mistake my focus on `diamonds` as an endorsement of the diamond industry. In my opinion aesthetic diamonds are a morally dubious scam.

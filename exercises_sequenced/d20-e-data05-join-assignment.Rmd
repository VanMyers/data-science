...
*Reading*: [Join Data Sets](https://rstudio.cloud/learn/primers/4.3) *Topics*: Welcome, mutating joins, filtering joins, Binds and set operations *Reading Time*: \~30 minutes
...
-   The rows were in different order so the surnames and instrument don't match the name and band
...
Use `expand_grid` to create a `criteria` dataframe with the `month` equal to `8, 9` and the airport identifiers in `dest` for the San Francisco, San Jose, and Oakland airports.
...
## TODO: Use expand_grid to create a criteria dataframe
  expand_grid(month = 8:9, dest = c("SFO", "SJC", "OAK"))
...
df_q4 <-
  flights %>%
## TODO: Use expand_grid to create a criteria dataframe
  semi_join(criteria, by = c("dest", "month"))

...
```{r q2-task}
# This is the solution I found obvious, not yet understanding you can rearrange with select itself
# front <- flights %>%
#   select(dest, origin, carrier)
# rest <- flights %>%
#   select(!c(dest, origin, carrier))
#   
# df_q2 <- c(front,rest)
...
```{r q4-task}
# BOS <- "LGA" # LGA is LaGuardia not Logan
flights %>%
  filter(dest == "BOS")
```

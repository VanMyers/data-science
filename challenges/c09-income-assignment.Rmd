...
-   Document your observations here.
    -   In all Massachussets counties except Nantucket, 2-person families have the lowest estimated median household income
    -   In these same counties, 3-person families have the next lowest estimated median household income
    -   Confidence intervals tend to be wider for larger n-person families. This is likely because they are fewer.
-   Can you confidently distinguish between household incomes in Suffolk county? Why or why not?
    -   No, each household size's confidence interval overlaps some portion of every other household size's confidence interval.
...
-   What *overall* trend do you see between `SE` and population? Why might this trend exist?
    -   Counties with higher populations tend to have a lower `SE`. This trend may exist because $SE = \frac{\sigma}{\sqrt{n}}$ meaning a higher sample size will reduce the `SE` if the standard deviation does not also change. The Census Bureau uses oversampling so sample size is not directly proportional to population size, but the two are still correlated.
-   What does this *overall* trend tell you about the relative ease of studying small vs large counties?
    -   We will have far greater confidence when studying large counties meaning it is relatively easier to test hypothesis about these counties.
...
-   Quite a few of the wealthiest counties are actually smaller. This could be explained by wealthy suburbs and vacation towns.
-   There is a visible increase in estimated income with population in counties with populations greater than 30,000
-   Clearly we are far from being able to draw any conclusions from this plot
-   New York county has the fewest large families by far. This is the county Manhattan is in.
-   I recognize this is ultimately not a very interesting visual because of how arbitrary the relative densities of counties are.
-   In terms of per county wealth, counties with major cities or their suburbs dominate. This isn't surprising because I explored totals and dense areas have more people.
-   It was surprising to see Oakland County in the suburbs of Detroit among the counties with the most wealth/
